...
class XmlGenerator {
    ...
    public function get_images(){
       $listItems = array();
        try{
            $DBH = new PDO("mysql:host=localhost;dbname=".DBNAME.";charset=".DBENCODING."", "".DBUSER."", "".DBPASS."");
            $q = "SELECT virtuemart_media_id as id,file_title as name, file_url as url FROM ".$this->_dbPrefix."medias WHERE file_mimetype='image/jpeg' OR file_mimetype='image/png' OR file_mimetype='image/gif'";
            $list =  $DBH->query($q) or die("failed getting images!");
            while($c = $list->fetch(PDO::FETCH_ASSOC)){
    			$imgtmp = str_replace('images/stories/virtuemart/product/','',$c['url']);
                $listItems[$c['id']] = $this->e($imgtmp);
            }
            $DBH = null;
        }catch(PDOEXCEPTION $e){
            echo $e->getMessage();
